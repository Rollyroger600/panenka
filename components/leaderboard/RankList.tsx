  ...
  return (
    <div className="rounded-xl bg-[#161616] border border-[#2a2a2a] overflow-hidden">
      {isSingle ? (
        ...
        <>
          <div className="grid grid-cols-[3.5rem_1fr_3rem_3.5rem_3rem] gap-1 px-3 py-2 bg-[#111] text-[10px] text-[#888] uppercase tracking-wide">
            <span>#</span>
            <span>Naam</span>
          ...
          {participants.map((p, i) => {
            ...
            const isCurrent = p.initials === currentInitials
            const delta = positionDeltas?.[p.initials]
            const ppt = p.tokensUsed > 0 ? (p.total / p.tokensUsed) : 0
            return (
              <div
                key={p.initials}
                className={`grid grid-cols-[3.5rem_1fr_3rem_3.5rem_3rem] gap-1 px-3 py-2.5 border-t border-[#1a1a1a] items-center ${
                  isCurrent ? 'bg-[#FF6B00]/10' : ''
                }`}
              >
                <div className="flex items-center gap-0.5">
                ...
                <span className="text-xs text-[#888] text-right">{p.tokensUsed}</span>
                <span className="text-xs text-[#888] text-right">{ppt > 0 ? ppt.toFixed(2) : '—'}</span>
              </div>
            )
          })}
        </>
