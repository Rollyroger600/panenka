...
export async function POST(req: NextRequest) {
  ...
  const { msgId, optionIndex } = await req.json()
  if (!msgId || optionIndex === undefined || typeof optionIndex !== 'number') {
    return NextResponse.json({ error: 'Ongeldige invoer' }, { status: 400 })
  }

  const pollOptions = await chatUpdatePoll(msgId, optionIndex, initials)
